package com.pdf.image.generator.imageUtil;
...
/**
 * This is a utility class for performing basic functions on an image, such as
 * retrieving, resizing, cropping, and saving.
 * 
 * @version 1.0
 * @author James H.
 */
public class Image {
	 ...
	 * @param img
	 */
	public Image(BufferedImage img, ImageType sourceType) {
		this.img = img;
		this.sourceType = (sourceType == null ? ImageType.UNKNOWN : sourceType);
	}
	...
	/**
	 * Generate a new Image object resized to a specific width, maintaining the same
	...
	/**
	 * Useful function to crop and resize an image to a square. This is handy for
	 * thumbnail generation.
	 * 
	 * @param width        Width of the resulting square
	 * @param cropEdgesPct Specifies how much of an edge all around the square to
	 *                     crop, which creates a zoom-in effect on the center of the
	 *                     resulting square. This may be useful, given that when
	 *                     images are reduced to thumbnails, the detail of the focus
	 *                     of the image is reduced. Specifying a value such as 0.1
	 *                     may help preserve this detail. You should experiment with
	 *                     it. The value must be between 0 and 0.5 (representing 0%
	 *                     to 50%)
	...
	public Image getResizedToSquare(int width, double cropEdgesPct) {
		if (cropEdgesPct < 0 || cropEdgesPct > 0.5)
			throw new IllegalArgumentException(
					"Crop edges pct must be between 0 and 0.5. " + cropEdgesPct + " was supplied.");
		if (width > getWidth())
			throw new IllegalArgumentException("Width " + width + " exceeds width of image, which is " + getWidth());
	 ...
	 * @throws ExecutionControl.NotImplementedException Not implemented in this class
	 */
	 ...
	 * this method will attempt to write the image of that type, IF it is supported.
	 * Otherwise JPG is the default. If the supplied File argument does not include
	 * an extension, this method will attempt to write the image in the type of the
	 * original source image, IF it is supported. Otherwise JPG is the default. This
	 * method will overwrite a file that exists with the same name
	 * 
	 ...
	 *          extension appended if none was supplied (i.e. if the method chose
	 *          the image type), or will have ".jpg" appended if the supplied
	...
	/**
	 * Write image to a file, specify image type This method will overwrite a file
	...
	public void writeToFile(File file, String type) throws IOException
	{
		...
		param.setCompressionQuality(0.07F); // Highest quality
	...
	public void writeToJPGNew(File file, float quality) throws IOException {
		...
		IIOImage outputImage = new IIOImage(this.img, null, null);
